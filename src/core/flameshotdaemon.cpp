...
std::vector<PaddleOCR::OCRPredictResult> recognize_w(cv::Mat);
std::vector<PaddleOCR::OCRPredictResult> ocr_recognize(std::string image_dir);// 只返回一张图片的结果
...
void FlameshotDaemon::attachPin(const QPixmap& pixmap, QRect geometry)
{
    ...
    cv::cvtColor(cvimg, cvimg, cv::COLOR_RGBA2BGR);//****************

    // std::string path_img = "zi4.png";
    // std::vector<OCRPredictResult> ocr_result = ocr_recognize(path_img);
    // cv::Mat mat_img = cv::imread(path_img);
    std::cout<<cvimg.size()<<std::endl;
    std::vector<OCRPredictResult> ocr_result = recognize_w(cvimg);
    std::vector<std::vector<int>> box;   // bounding box [[0,1],[2,3]]  // 这是一个框
    std::string str_re;                                                 // 这是一个字符串
    
    pinWidget->ocr_labels.update(ocr_result);
    pinWidget->ocr_labels.show();
    pinWidget->display_TickLabel(pinWidget);    // 设置父控件
