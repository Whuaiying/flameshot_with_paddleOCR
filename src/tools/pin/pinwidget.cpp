  ...
  , m_shadowEffect(new QGraphicsDropShadowEffect(this))
{
    ...
    setGraphicsEffect(m_shadowEffect);       // 设置阴影
...
void PinWidget::paintEvent(QPaintEvent* event)
{
    if (m_sizeChanged) {
        ...
        //
        // ocr_labels->labels
        ocr_labels.change_size(m_currentStepScaleFactor, m_scaleFactor);
        // ocr_labels.show();
        
        //------------------------------------
        m_label->setPixmap(pix);
        adjustSize();
        display_TickLabel(this);
...
void PinWidget::display_TickLabel(QWidget* parent)
{
    for (int i=0;i<ocr_labels.labels.size();i++)
    {
        ocr_labels.labels[i]->setParent(parent);//调用函数设置父对象
    }
